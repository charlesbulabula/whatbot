...
test('stats page renders every period, and the CSV export opens in Excel', async () => {
  for (const days of [7, 30, 90]) {
    ...
    assert.equal((html.match(/class="pt"/g) || []).length, days, 'one point per day on the revenue chart');
  }
  ...
  assert.match(text, /^\uFEFFreference;date;status;payment;customer/);
...
test('the shop pages manage areas, promo codes and settings', async () => {
  ...
  const postTab = (tab, fields) =>
    post('/admin/settings', new URLSearchParams({ tab, ...fields }).toString());

  assert.equal((await postTab('shop', {
    name: 'Epices Test', minOrder: '2500', defaultDeliveryFee: '2000', weeklyCapacity: '0',
  })).status, 302);
  assert.equal((await postTab('payment', {
    momoEnabled: '1', momoOrange: '+243 899 00 00 00', momoHolder: 'Charles B.', cashEnabled: '1',
  })).status, 302);
  assert.equal((await postTab('alerts', { adminNotifyNumber: '243899000000', emailAlerts: '1' })).status, 302);
  assert.equal((await postTab('hours', { open_1: '1', from_1: '08:00', to_1: '18:00' })).status, 302);

  const saved = settings.get();
  assert.equal(saved.minOrder, 2500);
  assert.equal(saved.momoOrange, '243899000000');
  assert.equal(saved.cashEnabled, true);
  assert.equal(saved.adminNotifyNumber, '243899000000');
  assert.deepEqual(saved.hours[1], { open: '08:00', close: '18:00' });
  assert.equal(saved.hours[2], null, 'a day left unticked is closed');

  // One tab does not clobber another: the shop name survived the later saves.
  assert.equal(saved.name, 'Epices Test');
