...
export function loyaltyPage(L, locale, data) {
  const { tab = 'balances', totals, holders, entries, referral, referrers, rules, theme, flash, role = 'owner', waHealth = null } = data;
...
${section(L.topReferrers, referrers.length
  ...
  return layout(L, { role, waHealth, title: L.navLoyalty, active: 'loyalty', body, flash, theme });
}
...
export function expensesPage(L, locale, data) {
  const { from, to, rows, total, revenue, byCategory, days, theme, flash, role = 'owner', waHealth = null } = data;
...
${section(`${L.expenses} — ${from} → ${to}`, rows.length
  ...
  return layout(L, { role, waHealth, title: L.navExpenses, active: 'expenses', body, flash, theme });
}

/* ------------------------------- audit log ------------------------------ */

export function auditPage(L, locale, { rows, total, page, pageSize, theme , role = 'owner', waHealth = null }) {
  const list = rows
...
${pager(L, { page, pageSize, total, url: (p) => `/admin/audit?page=${p}` })}`;
  return layout(L, { role, waHealth, title: L.navAudit, active: 'audit', body, theme });
}

/* ------------------------------- broadcast ------------------------------ */

export function broadcastPage(L, locale, { audiences, lastResult, theme, flash, flashTone , role = 'owner', waHealth = null }) {
...
<td class="num tabnum">${a.count}</td></tr>`).join('')))}`;
  ...
  return layout(L, { role, waHealth, title: L.navBroadcast, active: 'broadcast', body, flash, flashTone, theme });
}

/* ----------------------------- global search ---------------------------- */

export function searchPage(L, locale, { query, results, theme , role = 'owner', waHealth = null }) {
...
${coupons.length ? section(L.coupons, table([L.couponCode, L.couponKind, { label: L.couponValue, num: true }], couponRows)) : ''}`;
  ...
  return layout(L, { role, waHealth, title: L.searchTitle, active: '', body, theme, search: query });
}
